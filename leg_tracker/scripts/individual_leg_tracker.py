...
class ObjectTracked:
    ...
    def __init__(self, x, y, now, confidence): 
        ...
        # A better method would be to use data-driven EM to find the params. 
        ...
        # Observation model. Can observe pos_x and pos_y (unless person is occluded - we deal with this later). 
...
class KalmanMultiTracker:
    ...
    def match_detections_to_tracks_global_nearest_neighbour(self, objects_tracked, objects_detected):
        ...
        for detect in objects_detected:
            ...
            # If the detection is within range of at least one person track, add it as an eligible detection in the munkres matching 
    ...
    def detected_clusters_callback(self, detected_clusters_msg):
        ...
        propagated = copy.deepcopy(self.objects_tracked)
        for track in propagated:
        ...
        matched_tracks = self.match_detections_to_tracks_global_nearest_neighbour(propagated, detected_clusters)

        # Update tracks with new observations 
        tracks_to_delete = set()
        for idx, propagated_track in enumerate(propagated):
            track = self.objects_tracked[idx]  # The corresponding non-propagated track
            if propagated_track in matched_tracks:
                matched_detection = matched_tracks[propagated_track]
            ...
            else:  # propagated_track not matched to a detection
        ...
        for track in tracks_to_delete:
            track.deleted = True  # Because the tracks are also pointed to in self.potential_leg_pairs, we have to mark them deleted so they can be deleted from that set too
            self.objects_tracked.remove(track)

        # If detections were not matched, create a new track  
        for detect in detected_clusters:
            if detect not in matched_tracks.values():
    ...
    def publish_tracked_objects(self, now):
        ...
        if not transform_available:
            rospy.loginfo("Person tracker: tf not available. Not publishing people")
        else:
        ...
        if not transform_available:
            ...
            for person in self.people_tracked:
                ...
                if self.publish_occluded or leg_1.seen_in_current_scan or leg_2.seen_in_current_scan:
                    ...
                    marker.pose.position.x = ps.point.x
                    marker.pose.position.y = ps.point.y
                    for i in range(2):  # publish two markers per person: one for body and one for head
