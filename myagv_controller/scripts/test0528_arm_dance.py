...
class CVControl:
    def __init__(self):
        ...
        self.velocity_publisher = rospy.Publisher("/cmd_vel", Twist, queue_size=10) #quesize = 10
        self.coords_publisher = rospy.Publisher("/arm_camera/coords", Float64MultiArray, queue_size=10)
        self.angle_publisher = rospy.Publisher("/arm_camera/angle", Float64, queue_size=3)
        self.step_publisher = rospy.Publisher("/step_control", Int32, queue_size=1)
        self.detection_subscriber = rospy.Subscriber("/yolov5/detections", BoundingBoxes, self.detection_callback, queue_size=5, buff_size=2**24) #1 0529 # buff_size 추가 # 추가한 이유는 메시지 크기가 크기 때문에
        ...
        self.largest_area = 0
        self.largest_area_center = 640  # Image center horizontally - callback에서 검출되지만 검출되지 않을 때 튐 방지용
        self.detected = False
        ...
        self.coords1 = [-110.00,-167.90,375.20,-94.24,42.70,-179.79]
        self.coords2 = [28.80,223.90,346.20,-88.67,11.11,100.73]
        ''' 얼굴 인식 관련 params (0526 추가 - haracascade 정면인지 로직) '''
        self.front_face = False
        self.front_face_history = []

        ''' YOLO 관련 params (0528 추가) '''
        self.target_center = 640
    ...
    #         linear_velocity_y = 0.2  # 원형 경로를 위한 속도 조정 - 0.15
    ...
    def rotate_target(self):
        angular_velocity = 0.0008 * (self.target_center - self.largest_area_center)
        if angular_velocity == 0:
            angular_velocity = -0.01        
    ...
    def analyze_video(self):
        def move_robot_arm_back_and_forth():
            for _ in range(2):  # 2회 반복 (필요에 따라 조정 가능)
                print('시작시작시작시작시작시작시작시작시작시작시작시작시작시작시작')
                self.send_coords(self.coords1)
                rospy.sleep(1)
                self.send_coords(self.coords2)
                rospy.sleep(1)
        import threading
