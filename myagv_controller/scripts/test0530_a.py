...
from sensor_msgs.msg import wCompressedImage
...
class CVControl:
    def __init__(self):
        ...
        self.start_time_adjustment = None   #step 40에서 일정시간 지나면 step 50으로 넘어가게끔
    ...
    def adjust_robot_arm_and_agv(self):
        print("Moving at step 40")
        if self.start_time_adjustment is None:
            self.start_time_adjustment = time.time()
        ...
        # 10초가 경과했는지 확인하고 step을 50으로 업데이트
        if time.time() - self.start_time_adjustment > 10:
            self.update_step(50)
            self.start_time_adjustment = None  # 타이머 초기화
