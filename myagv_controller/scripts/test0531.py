...
class CVControl:
    def __init__(self):
        ...
        self.integral = 0
        self.previous_error = 0  # 이전 오류 값을 저장할 변수 추가
        ''' 감지 관련 params '''
        self.last_detected_positions = []  # Detected positions of people
        self.last_move_time = time.time()
        self.last_detection_time = time.time()  # 마지막 YOLO 검출 시간을 저장할 변수
        self.largest_area = 0
        self.largest_area_center = 0  # Image center horizontally - callback에서 검출되지만 검출되지 않을 때 튐 방지용
        self.detected = False
        ...
        self.dead_zone_width = 80 # 0531 수정 (100 -> 80)
    ...
    def update_step(self, step):
        # if self.is_moving_callback == False:
        self.step = step
    ...
    def move_toward_target(self):
        ...
        Kp = 0.0015  # 기존 Kp보다 낮춤
        Ki = 0   # 기존 Ki보다 낮춤 0.00000005
        Kd = 0.00015   # 새로운 미분 계수 추가 0.0005
        ...
        angular_velocity = Kp * error + Ki * self.integral + Kd * derivative
        ...
        linear_velocity = -0.000015 * (self.largest_area - target_area) * 2
    ...
    #     angular_velocity = Kp * error + Ki * integral
    #     linear_velocity = -0.000002 * (self.largest_area - target_area) * 2
    #     linear_velocity = max(-0.25, min(0.25, linear_velocity))        
        ...
        #     angular_velocity = 0
        # else:
        #     angular_velocity = 0.0012 * (self.target_center - self.largest_area_center) # 0.0006
    ...
    def rearrangement(self):
        ...
        if abs(self.largest_area_center - self.target_center) <= 100:
            angular_velocity = 0
        else:
