...
# 1. rosrate 30hz로 수정하여 영상최적화
# 2. 끝나는 자세 판단 로직을 통한 녹화 종료
# 3. 프레임 손실은 방지하기 위한 queue_size 조정
# 4. 비디오 녹화 중간에 타이머 한줄 추가
...
step0_map3_path = os.path.join(parent_path, 'mp3', 'step0.mp3')
step10_mp3_path = os.path.join(parent_path, 'mp3', 'step10.mp3')
step30_mp3_path = os.path.join(parent_path, 'mp3', 'step30.mp3')
step40_mp3_path = os.path.join(parent_path, 'mp3', 'step40.mp3')
step50_mp3_path = os.path.join(parent_path, 'mp3', 'step50.mp3')
step60_mp3_path = os.path.join(parent_path, 'mp3', 'step60.mp3')
step100_mp3_path = os.path.join(parent_path, 'mp3', 'step100.mp3')
...
class CVControl:
    def __init__(self):
        ...
        self.camera_subscriber = rospy.Subscriber("/arm_camera/image/compressed", CompressedImage, self.camera_callback, queue_size=10) #1
    ...
    def step_callback(self, msg):
        self.step = msg.data
        self.play_mp3_and_log()  # 새로운 스텝을 수신하면 MP3 재생


    def play_mp3_and_log(self, mp3_path):
        """
        MP3 파일을 재생하고, 콘솔에 로그를 출력하는 함수
        """
        print(f"Playing MP3: {mp3_path}")
        pygame.mixer.init()
    ...
    # 추가 0525 (Hara 기반 front_face 판단 로직 변경)
    ...
    def move_toward_target(self):
        target_center = 640
        target_area = 150000 #50000
        dead_zone_width = 100
        target_area_threshold = 3000  # 타겟 면적 허용 오차
        ...
        linear_velocity = -0.000002 * (self.largest_area - target_area) #-0.0000045
    ...
    #     rospy.sleep(1)
    #     if not self.current_landmarks or not feet_landmarks_visible(self.current_landmarks):
    ...
    def rotate_target(self):
        target_center = 640
        angular_velocity = 0.001 * (target_center - self.largest_area_center) # 0.0015
        if angular_velocity==0:
            angular_velocity = -0.01        
        linear_velocity_y = 0.12  # 원형 경로를 위한 속도 조정 - 0.15
        self.send_command(0, angular_velocity, linear_velocity_y)

        '''rospy.sleep(1)  # 1초 동안 움직임
        self.send_command(0, 0, 0)  # 멈춤
    ...
    def run(self):
        rate = rospy.Rate(30)  # 10Hz 루프 속도 설정
        # rospy.Rate(10)을 설정한 이유는 10Hz로 루프를 실행하여 1초에 10번의 루프를 실행하도록 하기 위함
        ...
        while not rospy.is_shutdown() and self.count < 4:
            ...
            if self.step != self.previous_step:
                if self.step == 0:
                    ...
                    self.play_mp3_and_log(step100_mp3_path)
            ...
            if self.step == 0:
                # self.play_mp3_and_log(step0_map3_path)
                self.step_publisher.publish(0)
                self.move_robot_arm(-5) #추가 0525 (로봇암 initial 각도 조절)
                if self.is_person_detected():
                    self.initiate_movement()
                    self.step = 10
                # elif self.use_leg_tracker:
                #     self.follow_person()
                ...
                # self.play_mp3_and_log(step30_mp3_path)
                self.move_robot_arm(-15) #추가 0525 (로봇암 각도 조절)
