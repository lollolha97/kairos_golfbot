# 키를 기준으로 거리를 조절하는 로직 변경 (하지만 아직 키 기반은 제어가 불안정하다.)
...
class CVControl:
    def __init__(self):
        ...
        self.largest_height_center = 320  # Image center horizontally
        self.count = 0
    ...
    def process_detections(self, data):
        ...
        for box in data.bounding_boxes:
            if box.Class == "person" and box.probability > 0.2:
                center, height = self.calculate_box_center_and_height(box)
                if height > self.largest_height:
                    self.update_detected_person(center, height, current_time)

    def calculate_box_center_and_height(self, box):
        centerX = (box.xmin + box.xmax) // 2
        height = box.ymax - box.ymin
        return centerX, height

    def update_detected_person(self, center, height, current_time):
        self.detected = True
        self.largest_height = height
        self.largest_height_center = center
    ...
    def move_toward_target(self):
        ...
        linear_velocity = -0.002 * (self.largest_height - target_height)
