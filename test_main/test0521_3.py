...
class CVControl:
    ...
    def follow_person(self):
        if self.target_position:
            ...
            if abs(angle_to_target) > math.radians(5):
                cmd_vel_msg.angular.z = -2 * (angle_to_target / math.pi)  # Scale angular speed by angle
            else:
    ...
    def move_at_step_thirty(self):
        ...
        while not (self.is_person_detected() and abs(self.largest_area_center - 320) <= 20) and not rospy.is_shutdown():
            if self.use_leg_tracker:
                self.follow_person()
            else:
                self.rotate_target()
