...
class CVControl:
    ...
    def move_at_step_thirty(self):
        ...
        while not rospy.is_shutdown():
            if not self.is_person_detected():
                self.stop_robot()
            if self.is_person_detected() and not self.front_face:
                self.rotate_target()
            elif self.is_person_detected() and self.front_face:
                self.stop_robot()
                rospy.sleep(0.5)
                break
            rospy.sleep(1)
