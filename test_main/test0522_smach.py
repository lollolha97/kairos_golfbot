...
class CVControl:
    ...
    def move_at_step_thirty(self):
        print("Moving at step 30 until person is detected and centered")
        move_cmd = Twist()
        while not rospy.is_shutdown():
            if self.is_person_detected() and abs(self.largest_area_center - 320) <= 20:
                break  # Person is detected and centered, break the loop
    ...
    def adjust_robot_arm_and_agv(self):
        ...
        while True:
            try:
                image_msg = rospy.wait_for_message("/arm_camera/image/compressed", CompressedImage, timeout=5)
    ...
    def camera_callback(self, data):
        rospy.loginfo(f"Is recording: {self.is_recording}, Use leg tracker: {self.use_leg_tracker}, Camera detected: {self.detected}")
        if self.is_recording:
            try:
...
class DetectPerson(smach.State):
    ...
    def execute(self, userdata):
        rospy.loginfo('Executing state DetectPerson')
        if self.control.is_person_detected():
            return 'detected'
        return 'not_detected'


class MoveToPerson(smach.State):
    def __init__(self, control):
        smach.State.__init__(self, outcomes=['arrived', 'not_arrived'])
        self.control = control

    def execute(self, userdata):
        rospy.loginfo('Executing state MoveToPerson')
        self.control.move_toward_target()
        if self.control.evaluate_stopped_condition():
            return 'arrived'
        return 'not_arrived'
...
class AdjustRobot(smach.State):
    ...
    def execute(self, userdata):
        rospy.loginfo('Executing state AdjustRobot')
        self.control.adjust_robot_arm_and_agv()
        return 'adjusted'


class RecordVideo(smach.State):
    def __init__(self, control):
        smach.State.__init__(self, outcomes=['recorded'])
        self.control = control

    def execute(self, userdata):
        rospy.loginfo('Executing state RecordVideo')
        self.control.start_video_recording()
        return 'recorded'


class AnalyzeVideo(smach.State):
    def __init__(self, control):
        smach.State.__init__(self, outcomes=['analyzed'])
        self.control = control

    def execute(self, userdata):
        rospy.loginfo('Executing state AnalyzeVideo')
        self.control.analyze_video()
        return 'analyzed'
...
def main():
    ...
    with sm:
        smach.StateMachine.add('RESET', Reset(control), 
                               transitions={'done':'DETECT_PERSON'})

        smach.StateMachine.add('DETECT_PERSON', DetectPerson(control), 
                               transitions={'detected':'MOVE_TO_PERSON', 
                                            'not_detected':'DETECT_PERSON'})

        smach.StateMachine.add('MOVE_TO_PERSON', MoveToPerson(control), 
                               transitions={'arrived':'ADJUST_ROBOT', 
                                            'not_arrived':'MOVE_TO_PERSON'})

        smach.StateMachine.add('ADJUST_ROBOT', AdjustRobot(control), 
                               transitions={'adjusted':'RECORD_VIDEO'})

        smach.StateMachine.add('RECORD_VIDEO', RecordVideo(control), 
                               transitions={'recorded':'ANALYZE_VIDEO'})

        smach.StateMachine.add('ANALYZE_VIDEO', AnalyzeVideo(control), 
                               transitions={'analyzed':'DETECT_PERSON'})
