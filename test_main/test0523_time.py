...
class CVControl:
    ...
    def rotate_target(self):
        ...
        linear_velocity_y = 0.2  # Adjust as needed for desired circular path
        self.send_command(linear_velocity_x, angular_velocity, linear_velocity_y)
    ...
    def move_at_step_thirty(self):
        ...
        while not rospy.is_shutdown():
            if self.is_person_detected() and not self.is_facing_forward:
                ...
                rospy.sleep(0.5)
                break  # Person is detected and facing forward, break the loop
    ...
    def adjust_robot_arm_and_agv(self):
        ...
        while self.step == 40:
            try:
                ...
                # Add a unique timestamp to each frame to avoid mismatch issues
                timestamp = rospy.Time.now().to_nsec()
                image_timestamped = self.add_timestamp_to_frame(frame_rgb, timestamp)
                
                results = pose.process(image_timestamped)
                ...
                rospy.logerr(f"Error in adjust_robot_arm_and_agv: {e}")

    def add_timestamp_to_frame(self, frame, timestamp):
        """Adds a unique timestamp to the frame to avoid MediaPipe timestamp mismatch errors."""
        # Create a new frame with the same data but a unique timestamp
        frame_with_timestamp = frame.copy()
        cv2.putText(frame_with_timestamp, str(timestamp), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
    ...
    def camera_callback(self, data):
        ...
        try:
            ...
            # Convert image back to compressed image message
